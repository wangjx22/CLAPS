...
def predict(net, data_loader):
    ...
    return total_labels.detach().numpy().flatten(), total_preds.detach().numpy().flatten(), total_prelabels.detach().numpy().flatten()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--downtask', default='BBBP', help='the dataset to train')
    ...
    for kind in att:
        for ran_split in datasplit:
            ...
            for epoch in range(1, epochs + 1):
                ...
                T, S, Y = predict(model, valid_loader)
                # compute preformence
                AUC = roc_auc_score(T, S)
                if AUC > max_valid_auc:
                    max_valid_auc = AUC
                    precision, recall, threshold = sklearn.metrics.precision_recall_curve(T, S)
                    PR_AUC = sklearn.metrics.auc(recall, precision)

                    AUCs = [epoch, AUC, PR_AUC]
                    save_AUCs(AUCs, valid_AUCs)

                    T, S, Y = predict(model, test_loader)
                    # compute preformence
                    AUC = roc_auc_score(T, S)
                    precision, recall, threshold = sklearn.metrics.precision_recall_curve(T, S)
                    PR_AUC = sklearn.metrics.auc(recall, precision)
