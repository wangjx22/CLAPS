...
class Transformer(nn.Module):
    def __init__(self, tgt_vocab_size, d_model, d_ff, d_k, d_v, n_heads, n_layers, precet, seq_len, dropout, trans_encoder, Emb_Pos_conder):
        ...
        # 自注意力机制选择遮盖
        self.attention = Encoder(self.d_model, self.d_ff, self.d_k, self.d_v, n_heads=2, n_layers=1) #.to(device)
    ...
    def forward(self, data1):
        ...
        # enc_outputs = self.src_emb(smiles)  #                                                                                                                                                                                                                                                                                                                        [batch_size, src_len, d_model]
        # enc_outputs = self.pos_emb(enc_outputs.transpose(0, 1)).transpose(0, 1)  # [batch_size, src_len, d_model]
        enc_outputs = self.emb_pos(smiles) # 初始化原始向量

        # mask
        mask_outout, mask_slef_attn = self.attention(smiles, enc_outputs) # 获smiles的attention的向量
        # list addself
        ...
        # 将相应权重的字符设为0，以掩盖
        mask_smiles, index_mask = creat_mask_matrix(mask, smiles, self.precet)
...
def creat_mask_matrix(mask, smiles, precet):
    ...
    # 新建一个标记矩阵将未被掩盖的都设为0，在最后计算loss的时候统一将索引为0 的不计算梯度
    if precet == 'Roulette':
        for i in range(raw):
            for j in range(line):
        ...
        for i in range(raw):
            for j in range(line):
                # largest =  true 即返回最大值
                if mask[i][j] == 0 or j == line:
                    values, indices = torch.topk(mask[i, :j], int(math.ceil(precet * j)), largest=False,
                                                 sorted=False)
                    one[i][indices] = 0
                    index_one[i][indices] = smiles[i][indices]
                    # soft = F.softmax(mask[i][:j], dim=0)
                    # for num in range(j):
                    #     if random.random() < (1 - soft[num]):
                    #         one[i][num] = 0
                    #     else:
                    #         continue
                    break
