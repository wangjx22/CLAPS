...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=300, type=int, help='Number of images in each mini-batch')
    ...
    print(args)
    temperature, k, datafile = args.temperature, args.k, args.datafile
    ...
    model = Transformer(tgt_vocab_size=None, d_model=d_model,
                        ...
                        precet=precet, seq_len=src_seq_len, dropout=dropout, trans_encoder=coder, Emb_Pos_conder=emb_pos)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if train_loss < minloss:
            train_loss = minloss
            torch.save(emb_pos.state_dict(), 'data/pretrain/model/Roulette_' + emb_pos_name_pre + '_emb_pos_epoch_' + str(epoch) + '.pkl')
            torch.save(coder.state_dict(), 'data/pretrain/model/Roulette_' + coder_name_pre + '_encoder_epoch_' + str(epoch) + '.pkl')
