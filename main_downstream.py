    ...
    for kind in att:
        for ran_split in datasplit:
            ...
            for epoch in range(1, epochs + 1):
                ...
                plt.scatter(Y[:, 0], Y[:, 1], 20, total_labels)  # labels为每一行对应标签，20为标记大小
                plt.savefig('results/' + kind + '_' + args.downtask + 'train' + str(epoch) + '.png')  # 保存图片
                plt.clf()
                ...
                if AUC > max_valid_auc:
                    ...
                    plt.scatter(Y[:, 0], Y[:, 1], 20, T)  # labels为每一行对应标签，20为标记大小
                    plt.savefig('results/' + kind + '_' + args.downtask + 'valid' + str(epoch) + '.png')  # 保存图片
                    plt.clf()
                ...
                if AUC > max_test_auc:
                    ...
                    plt.scatter(Y[:, 0], Y[:, 1], 20, T)  # labels为每一行对应标签，20为标记大小
                    plt.savefig('results/' + kind + '_' + args.downtask + 'test' + str(epoch) + '.png')  # 保存图片
                    plt.clf()
